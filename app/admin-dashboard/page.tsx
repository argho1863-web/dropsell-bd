...
interface Product{_id:string;name:string;price:number;category:string;description:string;image:string;allowedPayments:string;}
...
const[pPayment,setPPayment]=useState<"both"|"cod"|"paynow">("both");
...
const openAdd=()=>{setEditingProduct(null);setPName("");setPPrice("");setPCat(categories[0]?.slug||"");setPDesc("");setPImg("");setPCloudId("");setPPayment("both");setShowModal(true);};
const openEdit=(p:Product)=>{setEditingProduct(p);setPName(p.name);setPPrice(String(p.price));setPCat(p.category);setPDesc(p.description);setPImg(p.image);setPCloudId("");setPPayment((p.allowedPayments||"both") as "both"|"cod"|"paynow");setShowModal(true);};
...
setSaving(true);
const res=await fetch(editingProduct?`/api/products/${editingProduct._id}`:"/api/products",{method:editingProduct?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:pName,price:Number(pPrice),category:pCat,description:pDesc,image:pImg,cloudinaryId:pCloudId,allowedPayments:pPayment})});
...
const paymentLabel=(p:string)=>p==="both"?"COD + Pay Now":p==="cod"?"COD Only":"Pay Now Only";
const paymentColor=(p:string)=>p==="both"?"#16a34a":p==="cod"?"#d97706":"#7c3aed";
...
<span style={{fontSize:"10px",background:paymentColor(p.allowedPayments||"both")+"20",color:paymentColor(p.allowedPayments||"both"),padding:"2px 8px",borderRadius:"99px",fontWeight:"600"}}>{paymentLabel(p.allowedPayments||"both")}</span>
</div>
<p style={{margin:"0 0 2px",fontWeight:"600",fontSize:"13px",color:"#111827",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</p>
...
<div>
<label style={{display:"block",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"8px"}}>Payment Method *</label>
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"8px"}}>
{([{v:"both",l:"Both",d:"Customer can choose",c:"#16a34a"},{v:"cod",l:"COD Only",d:"Cash on delivery",c:"#d97706"},{v:"paynow",l:"Pay Now Only",d:"Online payment",c:"#7c3aed"}] as const).map(opt=>(
<button key={opt.v} type="button" onClick={()=>setPPayment(opt.v)} style={{padding:"10px 8px",borderRadius:"10px",border:`2px solid ${pPayment===opt.v?opt.c:"#e5e7eb"}`,background:pPayment===opt.v?opt.c+"15":"white",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
<p style={{margin:"0 0 2px",fontSize:"12px",fontWeight:"700",color:pPayment===opt.v?opt.c:"#374151"}}>{opt.l}</p>
<p style={{margin:0,fontSize:"10px",color:pPayment===opt.v?opt.c:"#9ca3af"}}>{opt.d}</p>
</button>
))}
</div>
</div>
